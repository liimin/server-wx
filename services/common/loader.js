...
!function(){
    ...
    tokenManager.on('token', info => {
        ...
        const { appid ,secret,token } = info
        fetch_content().then(content=>{
            const where = {'temple_id': content.id}
            wxinfo.findOrCreate({
                where, 
                defaults: {appid,'appsecret':secret,'access_token':token,'start_tm':Date.now(),'valid_tm':7200-120,'createtime':new Date(),'updatetime':new Date()}
                }).spread((user, created) => {
                if(created === false) {
                    wxinfo.update( {'access_token':token,'start_tm':Date.now(),'updatetime':new Date()},{ where }).then(_=>{
